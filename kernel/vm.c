...
void
uvmunmap(pagetable_t pagetable, uint64 va, uint64 npages, int do_free, int procId)
{
...
uvmalloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz, int xperm, int procId)
{
  ...
  for(a = oldsz; a < newsz; a += PGSIZE){
    mem = kalloc();
    if(mem == 0){
      uvmdealloc(pagetable, a, oldsz, procId);
      return 0;
    }
    memset(mem, 0, PGSIZE);
...
uint64
uvmdealloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz, int procId)
{
  if(newsz >= oldsz)
    return oldsz;
  ...
  if(PGROUNDUP(newsz) < PGROUNDUP(oldsz)){
    ...
    uvmunmap(pagetable, PGROUNDUP(newsz), npages, 1, procId);
  }
...
void
uvmfree(pagetable_t pagetable, uint64 sz, int procId)
{
  // printf("uvmfree Entry\n");
  if(sz > 0){
    uvmunmap(pagetable, 0, PGROUNDUP(sz)/PGSIZE, 1, procId);
  }
  // printf("asdf");
  freewalk(pagetable);
  // printf("uvmfree Exit\n");
...
copyout(pagetable_t pagetable, uint64 dstva, char *src, uint64 len, int procId)
{
  ...
  while(len > 0){
    ...
    if((flags & PTE_COW)){
      ...
      removeLive(VA2VPN(va0), procId, pte);
...
int assignPagesOnWrite(pagetable_t p, int procId){
  ...
  acquireSlock();
  // printf("pid: %d vpn: %d swappedbit: %d valid bit: %d\n", procId, VA2VPN(va), (*pte) & PTE_SWAPPED, (*pte) & PTE_V);
  ...
  // printf("valid1: %d\n",(*pte & PTE_V));
  if(flags & PTE_SWAPPED){
    // printf("swap in from swap on write en\n");
    // printf("swapped and valid bit before swap: s %d v: %d\n", *pte & PTE_SWAPPED, *pte & PTE_V);
    swapIn(VA2VPN(va),procId, pte);
    // printf("swap in from swap on write ex\n");
    // printf("swapped and valid bit after swap: s %d v: %d\n", *pte & PTE_SWAPPED, *pte & PTE_V);
  }
  ...
  flags &= ~PTE_COW;
  // printf("vpn: %d\n",VA2VPN(va));
  removeLive(VA2VPN(va), procId, pte);
